...
Strategy: Add a new column to the origin dataset named `steps_completed`
that replaces missing values with the rounded average of the 5-minute
interval.
    
```{r, echo = TRUE}
activityData$steps_completed <- ifelse(is.na(activityData$steps), round(stepsPerIntervalAvg$steps[match(activityData$interval, stepsPerIntervalAvg$interval)], 0), activityData$steps)
```
...
```{r, echo = TRUE}
activityDataNoNA <- data.frame(steps = activityData$steps_completed, interval = activityData$interval, date = activityData$date)
...
Reviewing the histogram, the only bin that was impacted is the interval between
10000 and 12500 steps which grew from a frequency of 18 to a frequency of 26
(44%). Instead of replacing missing values with the mean, such as the mode or 
